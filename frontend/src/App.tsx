...
import { accountLabel, formatDateTime, humanize, incidentTitle, prioritizeIncidents, relativeTime, scorePercent } from './utils'
...
export default function App() {
  ...
  const activeCapabilities = capabilityItems.filter((capability) => capability.enabled).length
  ...
  return <div className="app-shell">
    ...
    <div className="app-main"><header className="topbar"><button aria-label="Open navigation" className="icon-button menu-button" onClick={() => setMenuOpen(true)}><Icons.menu /></button><form className="global-search" onSubmit={(event) => { event.preventDefault(); setSection('incidents') }}><Icons.search /><label className="sr-only" htmlFor="global-search">Search incidents</label><input id="global-search" onChange={(event) => setFilters((current) => ({ ...current, q: event.target.value }))} placeholder="Search incidents, signals, or accounts" value={filters.q} /></form><div className="top-actions"><button className="button button-primary scan-button" disabled={!apiKey} onClick={() => setScanOpen(true)}><Icons.scan /><span>New scan</span></button><button aria-label="API settings" className="avatar-button" onClick={() => setSettingsOpen(true)}><span>{apiKey ? 'SO' : '?'}</span><i className={apiKey ? 'online' : ''} /></button></div></header>
      ...
      <main id="main-content">{!apiKey || !tenantId ? <ConnectionCard onConnect={connect} /> : <>
        ...
        {section === 'coverage' && <div className="page"><header className="page-header"><div><p className="eyebrow">Connected environment</p><h1>Coverage</h1><p>Accounts and detection capabilities reported by SentinelSME.</p></div><button className="button button-primary" onClick={() => setScanOpen(true)}><Icons.scan />New scan</button></header><div className="coverage-layout"><Panel><div className="panel-heading"><div><p className="eyebrow">Assets</p><h2>Protected accounts</h2></div><span>{accounts.length}</span></div>{loading ? <LoadingRows /> : accounts.length ? <div className="account-list">{accounts.map((account) => { const label = accountLabel(account); return <article key={account.id}><span className="account-avatar">{label.slice(0, 2).toUpperCase()}</span><div><strong>{label}</strong><span>{account.email || account.provider || account.id}</span><small>Last scan: {account.last_scan_at ? relativeTime(account.last_scan_at) : 'not reported'}</small></div><StatusBadge status={account.status || 'connected'} /></article> })}</div> : <EmptyState />}</Panel><Panel><div className="panel-heading"><div><p className="eyebrow">Controls</p><h2>{capabilities?.service || 'SentinelSME'} capabilities</h2></div><span>{activeCapabilities} active</span></div>{loading ? <LoadingRows /> : capabilityItems.length ? <><div className="capability-grid">{capabilityItems.map((capability) => <article key={capability.id}><span className={`capability-icon ${!capability.enabled ? 'disabled' : ''}`}><Icons.shield /></span><div><strong>{capability.name}</strong><p>{capability.description}</p><span className={!capability.enabled ? 'control-off' : 'control-on'}>{capability.enabled ? 'Active' : 'Unavailable'}</span></div></article>)}</div>{capabilities && <div className="capability-facts"><span>{capabilities.correlation_window_minutes} min correlation</span><span>{capabilities.source_types.length} source types</span><span>{capabilities.live_connectors.length} live connectors</span><span>{capabilities.privacy.stores_full_bodies || capabilities.privacy.stores_attachments ? 'Content retention enabled' : 'No full bodies or attachments stored'}</span></div>}</> : <EmptyState />}</Panel></div></div>}
      </>}</main>
    </div>
